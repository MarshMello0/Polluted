...
public static class Noise 
{
	...
	public static float[,] GenerateNoiseMap(int mapWidth, int mapHeight, int seed, float scale, int octaves, float persistance, float lacunarity, Vector2 offset, NormalizeMode normalizeMode, List<City> cities) 
	{
		...
		//This is used to work out the average height
		float heightTotal = 0;
...
//				city.name, city.position.x, city.position.z, worldPosition.x - 50, worldPosition.z - 50));
		...
		if (cityInChunk != null)
		{
			...
			float average = heightTotal / (mapWidth * mapHeight);
			...
			for (int xIndex = 0; xIndex < amountX; xIndex++)
			{
				for (int zIndex = 0; zIndex < amountZ; zIndex++)
				{
					...
					noiseMap[xIndex + Mathf.RoundToInt(cityStartX),zIndex + Mathf.RoundToInt(cityStartZ)] = average;
				}
			}
	...
	public static float GetHeightAtPoint(int mapWidth, int mapHeight, float mapX, float mapY, int seed, float scale, int octaves, float persistance, float lacunarity, Vector2 offset, NormalizeMode normalizeMode, City city)
	{
		...
		//Debug.Log(amountX + " | " + amountZ + " | " + noiseMap.GetLength(0) + " | " + noiseMap.GetLength(1));

		returnHeight = average;
		
		Debug.Log(returnHeight);
		//Need to add the working out the average from that height
		return returnHeight;
	}
}
